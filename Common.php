...
//Connection Settings @0-B9640CC5
$CCConnectionSettings = array (
  "catastro" => array(
    "Type" => "MySQL",
    "DBLib" => "MySQL",
    "Database" => "catastro_tdf",
    "Host" => "localhost",
    "Port" => "",
    "User" => "php_user",
    "Password" => "phpuser",
  ...
  "mesa" => array(
    "Type" => "MySQL",
    "DBLib" => "MySQL",
    "Database" => "catastro_tdf_mesa_desa",
    "Host" => "localhost",
    "Port" => "",
    "User" => "php_user",
    "Password" => "phpuser",
...
//Initialize Common Variables @0-93FCFB9C
...
//catastro Connection Class @-B2628BF6
class clsDBcatastro extends DB_Adapter
{
  function clsDBcatastro()
  {
    $this->Initialize();
  }
  ...
  function Initialize()
  {
    ...
    $this->SetProvider($CCConnectionSettings["catastro"]);
    parent::Initialize();
...
//End catastro Connection Class

//mesa Connection Class @-52E40469
class clsDBmesa extends DB_Adapter
{
  function clsDBmesa()
  {
    $this->Initialize();
  ...
  {
    global $CCConnectionSettings;
    $this->SetProvider($CCConnectionSettings["mesa"]);
    parent::Initialize();
    $this->DateLeftDelimiter = "'";
    $this->DateRightDelimiter = "'";
  }
...
//End mesa Connection Class

//unidades Connection Class @-ADDAAD51
class clsDBunidades extends DB_Adapter
{
  function clsDBunidades()
  {
    $this->Initialize();
  }
  ...
  {
    global $CCConnectionSettings;
    $this->SetProvider($CCConnectionSettings["unidades"]);
    parent::Initialize();
    $this->DateLeftDelimiter = "'";
    $this->DateRightDelimiter = "'";
  }
...
//End unidades Connection Class

//tdf_nuevo Connection Class @-0E18313C
class clsDBtdf_nuevo extends DB_Adapter
{
  function clsDBtdf_nuevo()
  {
    $this->Initialize();
  }
  ...
  {
    global $CCConnectionSettings;
    $this->SetProvider($CCConnectionSettings["tdf_nuevo"]);
    parent::Initialize();
    $this->DateLeftDelimiter = "'";
    $this->DateRightDelimiter = "'";
  }
...
//End tdf_nuevo Connection Class
...
//CCGetUserID @0-711FC845
function CCGetUserID()
{
  return CCGetSession("UID");
}
//End CCGetUserID

//CCGetGroupID @0-C1783C94
function CCGetGroupID()
{
  return CCGetSession("GID");
}
//End CCGetGroupID
...
//CCLoginUser @0-A180BFA1
function CCLoginUser($login, $password)
{
  CCLogoutUser();
  $db = new clsDBtdf_nuevo();
  $SQL = "SELECT usuario_id, perfil_id, usuario_clave FROM _usuarios WHERE usuario_login=" . $db->ToSQL($login, ccsText) . " AND usuario_clave=" . $db->ToSQL($password, ccsText);
  $db->query($SQL);
  $Result = $db->next_record();
  if ($Result) {
    CCSetSession("UID", $db->f("usuario_id"));
    CCSetSession("UserLogin", $login);
    CCSetSession("GID", $db->f("perfil_id"));
  }
  return $Result;
}
//End CCLoginUser

//CCLogoutUser @0-0B390164
