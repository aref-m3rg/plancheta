<?php
/**
 * Proxy autenticado para adjuntos de plano (imágenes / PDF) en PLANOS_ATTACHED_FILESYSTEM_ROOT.
...
require_once dirname(__FILE__) . DIRECTORY_SEPARATOR . 'scripts' . DIRECTORY_SEPARATOR . 'plano_adjunto_archivo_local.php';
...
if (!preg_match('/^[A-Za-z0-9._-]+\.(jpe?g|png|gif|pdf)$/i', $archivoParam)) {
	header('HTTP/1.1 400 Bad Request');
	header('Content-Type: text/html; charset=utf-8');
...
$fileReal = plano_adjunto_archivo_local_path_validated($archivoParam);
if ($fileReal === false) {
